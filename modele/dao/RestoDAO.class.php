...
class RestoDAO {
    ...
    public static function getTop4(): array {
        $lesObjets = array();
        try {            
            $requete = "SELECT AVG(note) AS noteMoyenne, r.idR, nomR, numAdrR, voieAdrR, cpR, villeR, latitudeDegR, longitudeDegR, descR, horairesR  
                       FROM resto r
    ...
    try {
        $requete = "SELECT DISTINCT r.* 
                    ...
                    WHERE 1=1 ";
        // on retire la ligne : LEFT JOIN proposer p ON r.idR = p.idR 

        // Construction dynamique des filtres
        if (!empty($extraitNomR)) {
            $requete .= " AND r.nomR LIKE :nomR";
        }
        if (!empty($voieAdrR)) {
            $requete .= " AND r.voieAdrR LIKE :voieAdrR";
        }
        if (!empty($cpR)) {
            $requete .= " AND r.cpR LIKE :cpR";
        }
        if (!empty($villeR)) {
            $requete .= " AND r.villeR LIKE :villeR";
        }
        if (!empty($tabIdTC)) {
            $ids = implode(',', array_map('intval', $tabIdTC));
            $requete .= " AND p.idTC IN ($ids)";
        }

        $requete .= " ORDER BY r.nomR";

        $stmt = Bdd::getConnexion()->prepare($requete);

        // Binding des paramètres
        if (!empty($extraitNomR)) {
            $stmt->bindValue(':nomR', "%$extraitNomR%", PDO::PARAM_STR);
        }
        if (!empty($voieAdrR)) {
        ...
        while ($enreg = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            throw new Exception("Erreur dans getAllMultiCriteres : " . $e->getMessage());
        }
